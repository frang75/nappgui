...
static const real32_t i_HOR_MARGIN = 10;

/*---------------------------------------------------------------------------*/

static void i_add(ArrSt(ViewItem) *items, const char_t *name, const color_t color)
{
    ViewItem *item = arrst_append1(items, ViewItem);
    item->name = name;
    item->color = color;
}
...
static ArrSt(ViewItem)* i_colors(void)
{
    ...
    i_add(items, "Label", kCOLOR_LABEL);
    i_add(items, "View", kCOLOR_VIEW);
    i_add(items, "Line", kCOLOR_LINE);
    i_add(items, "Link", kCOLOR_LINK);
    i_add(items, "Text", kCOLOR_TEXT);
    i_add(items, "SelText", kCOLOR_SELTEXT);
    i_add(items, "BgText", kCOLOR_BGTEXT);
    i_add(items, "BgSelText", kCOLOR_BGSELTEXT);
    i_add(items, "Header", kCOLOR_HEADER);
    i_add(items, "Row1", kCOLOR_ROW1);
    i_add(items, "Row2", kCOLOR_ROW2);
    i_add(items, "Alt1", color_alt(color_rgb(192, 255, 255), color_rgb(48, 112, 112)));
    i_add(items, "Alt2", color_alt(color_rgb(255, 192, 255), color_rgb(128, 48, 112)));
    i_add(items, "Alt3", color_alt(color_rgb(255, 255, 192), color_rgb(112, 112, 48)));
...
static void i_draw(DCtx *ctx, real32_t x, real32_t y, real32_t width, real32_t height, const ViewItem *item)
{
    ...
    draw_fill_color(ctx, kCOLOR_LABEL);
    draw_text(ctx, ekFILL, item->name, cx1, cy);
}
...
static void i_OnSize(App *app, Event *e)
{
    ...
    view_scrollbar_size(view, &scroller);
    cwidth = p->width - scroller.width;
    ...
    else
    {
        ...
        {
            cheight = p->height;
            cwidth += scroller.width;
        }
    }
    
    view_content_size(view, s2df(cwidth, cheight));
    view_update(view);
}
...
static Panel *i_panel(App *app)
{
    ...
    View *view = view_create(ekHSCROLL | ekVSCROLL);
